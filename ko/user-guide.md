...
### 노드 그룹에 노드 추가
동작하고 있는 노드 그룹에 노드를 추가할 수 있습니다. 노드 그룹 정보 조회 페이지의 노드 목록 탭을 클릭하면 현재 노드 목록이 출력됩니다. 노드 추가 버튼을 클릭하고 노드수를 입력하면 노드가 추가됩니다.

### 노드 그룹에서 노드 삭제
동작하고 있는 노드 그룹에서 노드를 삭제할 수 있습니다. 노드 그룹 정보 조회 페이지의 노드 목록 탭을 클릭하면 현재 노드 목록이 출력됩니다. 노드 목록 중 삭제할 노드를 선택하고 노드 삭제 버튼을 클릭하면 확인창이 나타납니다. 삭제할 노드 이름을 다시 한번 확인하고 확인 버튼을 클릭하면 노드가 삭제됩니다.

>[주의]
>삭제되는 노드에서 동작하고 있던 파드는 안전하게 처리되지 않습니다. 파드를 안전하게 처리하기 위해서는 해당 파드를 다른 노드로 옮기고(drain), 다른 파드가 해당 노드에 스케쥴링되지 않도록(cordon) 해야 합니다. 클러스터를 안정적으로 운영하기 위해 노드를 삭제하기 전 이와 같은 처리를 하시길 바랍니다. 안전한 노드 관리에 대한 좀 더 자세한 내용은 아래 문서를 참고하세요.

* [안전한 drain 처리](https://kubernetes.io/docs/tasks/administer-cluster/safely-drain-node/)
